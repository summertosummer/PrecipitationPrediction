...
if not os.path.exists('models1155_1683_9'):
    os.makedirs('models1155_1683_9')
if not os.path.exists('test1155_1683'):
    os.makedirs('test1155_1683')
if not os.path.exists('pred1155_1683'):
    os.makedirs('pred1155_1683')
if not os.path.exists('mae1155_1683'):
    os.makedirs('mae1155_1683')
if not os.path.exists('rmse1155_1683'):
    os.makedirs('rmse1155_1683')
...
def run_models(grid_y, grid_x):
    ...
    with open("models1155_1683_9/"+str(grid_x)+"_"+str(grid_y)+"_lin.pickle", "wb") as f:
        pickle.dump(lin, f)
    with open("models1155_1683_9/"+str(grid_x)+"_"+str(grid_y)+"_rf.pickle", "wb") as f:
        pickle.dump(rf, f)
    with open("models1155_1683_9/"+str(grid_x)+"_"+str(grid_y)+"_nnr.pickle", "wb") as f:
        pickle.dump(nnr, f)
    with open("models1155_1683_9/"+str(grid_x)+"_"+str(grid_y)+"_svm.pickle", "wb") as f:
        pickle.dump(svm, f)
    with open("models1155_1683_9/"+str(grid_x)+"_"+str(grid_y)+"_knn.pickle", "wb") as f:
        pickle.dump(knn, f)
...
check = open('ModelsInfo1155_1683.csv', 'w')
check.truncate()
check.write(str('Y'))
check.write(', ')
check.write(str('X'))
...
y2 = 1
for slice in range(1, 1150, 10):
    rain_models = rain_model[:, :, :, slice:slice + 10, :]
    for grid_y in range(0, 10): # for every y
        for grid_x in range(1, 1682): # for every x
            print('=================PLACE:', grid_x, y2, '=====================')

            flag = True
            for _ in range(15): # looping 15 times to find the best model
                try:
                    mae, rmse, predictions, test = run_models(y2, grid_x)
                    minRMSE = np.amin(rmse) # minimum RMSE from the new models
                    # total += 1
                    if best_rmse(minRMSE, y2, grid_x): # checking if it is the best one
            ...
            if flag:
                ...
                if best_mae(minMAE, y2, grid_x):
                    countMAE += 1
                if best_rmse(minRMSE, y2, grid_x):
                    countRMSE += 1

                # saving mae, rmse, prediction and target data
                np.savetxt('test1155_1683/' + str(grid_x) + '_' + str(y2) + '.csv', test.Y, delimiter=',', fmt='%10.5f')
                np.savetxt('pred1155_1683/' + str(grid_x) + '_' + str(y2) + '.csv', predictions, delimiter=',', fmt='%10.5f')
                np.savetxt('mae1155_1683/' + str(grid_x) + '_' + str(y2) + '.csv', mae, delimiter=',', fmt='%10.5f')
                np.savetxt('rmse1155_1683/' + str(grid_x) + '_' + str(y2) + '.csv', rmse, delimiter=',', fmt='%10.5f')
