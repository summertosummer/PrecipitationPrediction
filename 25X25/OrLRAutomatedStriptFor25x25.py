...
if not os.path.exists('or_models25_25_9'):
    os.makedirs('or_models25_25_9')
if not os.path.exists('or_test'):
    os.makedirs('or_test')
if not os.path.exists('or_pred'):
    os.makedirs('or_pred')
if not os.path.exists('or_mae'):
    os.makedirs('or_mae')
if not os.path.exists('or_rmse'):
    os.makedirs('or_rmse')
if not os.path.exists('or_features'):
    os.makedirs('or_features')
if not os.path.exists('or_pca'):
    os.makedirs('or_pca')
if not os.path.exists('or_coef'):
    os.makedirs('or_coef')
...
def run_models(grid_y, grid_x):
    ...
    np.savetxt('or_features/' + str(grid_x) + '_' + str(grid_y) + '.csv', out_data2, delimiter=',', fmt='%10.5f')
    ...
    np.savetxt('or_pca/' + str(grid_x) + '_' + str(grid_y) + '.csv', train, delimiter=',', fmt='%10.5f')
    # print(out_data.domain)
    ...
    with open("or_models25_25_9/"+str(grid_x)+"_"+str(grid_y)+"_lin.pickle", "wb") as f:
        pickle.dump(lin, f)
...
check = open('or_ModelsInfo25x25.csv', 'w')
check.truncate()
check.write(str('Y'))
check.write(', ')
check.write(str('X'))
...
for grid_y in range(1, 45): # for every y
    for grid_x in range(1, 66): # for every x
        ...
        if flag:
            ...
            np.savetxt('or_test/' + str(grid_x) + '_' + str(grid_y) + '.csv', test.Y, delimiter=',', fmt='%10.5f')
            np.savetxt('or_pred/' + str(grid_x) + '_' + str(grid_y) + '.csv', predictions, delimiter=',', fmt='%10.5f')
            np.savetxt('or_mae/' + str(grid_x) + '_' + str(grid_y) + '.csv', mae, delimiter=',', fmt='%10.5f')
            np.savetxt('or_rmse/' + str(grid_x) + '_' + str(grid_y) + '.csv', rmse, delimiter=',', fmt='%10.5f')
