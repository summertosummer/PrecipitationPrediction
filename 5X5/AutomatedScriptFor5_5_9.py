...
if not os.path.exists('models5_5_9'):
    os.makedirs('models5_5_9')
if not os.path.exists('test5_5'):
    os.makedirs('test5_5')
if not os.path.exists('pred5_5'):
    os.makedirs('pred5_5')
if not os.path.exists('mae5_5'):
    os.makedirs('mae5_5')
if not os.path.exists('rmse5_5'):
    os.makedirs('rmse5_5')

#access netcdf data file
netcdf_entire_dataset = Dataset("F:/dataset/summing_dataset5_5.nc", "r")
rain_models = netcdf_entire_dataset.variables['summing_models']

#read MAE and RMSE files
dfMAE = pd.read_csv('F:/dataset/MAE5_5.csv', header=None)
dfRMSE = pd.read_csv('F:/dataset/RMSE5_5.csv', header=None)
# print(dfRMSE.values[(dfRMSE[0] == 200) & (dfRMSE[1] == 300)])
...
def run_models(grid_y, grid_x):
    ...
    with open("models5_5_9/"+str(grid_x)+"_"+str(grid_y)+"_lin.pickle", "wb") as f:
        pickle.dump(lin, f)
    with open("models5_5_9/"+str(grid_x)+"_"+str(grid_y)+"_rf.pickle", "wb") as f:
        pickle.dump(rf, f)
    with open("models5_5_9/"+str(grid_x)+"_"+str(grid_y)+"_nnr.pickle", "wb") as f:
        pickle.dump(nnr, f)
    with open("models5_5_9/"+str(grid_x)+"_"+str(grid_y)+"_svm.pickle", "wb") as f:
        pickle.dump(svm, f)
    with open("models5_5_9/"+str(grid_x)+"_"+str(grid_y)+"_knn.pickle", "wb") as f:
        pickle.dump(knn, f)
...
def best_mae(minMAE, grid_y, grid_x):
    ...
    if df1.empty:
        df1 = dfMAE[(dfMAE[0] == grid_y) & (dfMAE[1] == grid_x)]
...
def best_rmse(minRMSE, grid_y, grid_x):
    ...
    print(df1.empty)
    if df1.empty:
        df1 = dfRMSE[(dfRMSE[0] == grid_y) & (dfRMSE[1] == grid_x)]
...
check = open('ModelsInfo5_5.csv', 'w')
check.truncate()
check.write(str('Y'))
check.write(', ')
check.write(str('X'))
...
countMAE = 0
countRMSE = 0
for grid_y in range(50, 231): # for every y
    for grid_x in range(50, 336): # for every x
        ...
        if flag:
            ...
            np.savetxt('test5_5/' + str(grid_x) + '_' + str(grid_y) + '.csv', test.Y, delimiter=',', fmt='%10.5f')
            np.savetxt('pred5_5/' + str(grid_x) + '_' + str(grid_y) + '.csv', predictions, delimiter=',', fmt='%10.5f')
            np.savetxt('mae5_5/' + str(grid_x) + '_' + str(grid_y) + '.csv', mae, delimiter=',', fmt='%10.5f')
            np.savetxt('rmse5_5/' + str(grid_x) + '_' + str(grid_y) + '.csv', rmse, delimiter=',', fmt='%10.5f')
