...
def plot_result(predictedValue, OriginalValue):
    ...
    plt.legend()
    plt.title('Plotting predicted and actual output for a particular area (Grid 23 X 23)')
    plt.show()

def plot_input(x, y):
    mpl.style.use('seaborn')
    plt.plot([], 'o', color='black', label='Prediction Models\' data')
    plt.plot(x, 'o')
    plt.plot(y, color='black', label='Real Data')
    plt.xlabel('Time')
    plt.ylabel('Rainfall (mm)')
    plt.legend()
    axes = plt.gca()
    axes.set_ylim([0, 6500])
    plt.title('Plotting selected input features and expected output for a particular area (Grid 23 X 23)')
    plt.show()
...
for grid_y in range (23, 45):
    for grid_x in range (23, 66):
        ...
        if flag:
            ...
            plot_result(predictions[0], test)
            plot_result(predictions[1], test)
            plot_result(predictions[2], test)
            plot_result(predictions[3], test)
